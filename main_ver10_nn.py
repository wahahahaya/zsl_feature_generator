...
class Res101(nn.Module):
    def __init__(self):
        ...
        self.conv1x1 = nn.Conv2d(2048, 312, (1, 1), 1)

    def forward(self, x):
        feature_map = self.resnet(x)
        feature_map = self.conv1x1(feature_map)  # 312, 8, 8
...
def cal_accuracy(net_cls, net_F, data_loader, device):
    scores = []
    labels = []
    ...
    for iteration, (img, label, attribute, feature) in enumerate(data_loader):
        ...
        feature_map = net_F(image)  # (B, 312, 64)
        attribute_map = torch.from_numpy(glove_map).float().to(device)  # (312, 300)

        key = feature_map
        query = attribute_map

        attention_map = torch.einsum('we,bem->bwm', query.permute(1, 0), key)  # (B, 300, 64)
        B, N1, N2 = attention_map.shape
        feature_attribute = F.max_pool1d(attention_map, 64)
...
def train():
    ...
    opt = optim.Adam(itertools.chain(net_F.parameters(), net_cls.parameters()), lr=1e-3)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            val_acc = cal_accuracy(net_cls, net_F, seen_loader, device)
