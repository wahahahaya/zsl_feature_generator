...
import classifier_images as classifier
...
def build_data():
    ...
    train_label = torch.from_numpy(image_label[trainval_loc]).long()
    test_unseen_feature = torch.from_numpy(_test_unseen_feature).float()
    test_unseen_feature.mul_(1/mx)
    test_label_unseen = torch.from_numpy(image_label[test_unseen_loc]).long()
    test_seen_feature = torch.from_numpy(_test_seen_feature).float()
    test_seen_feature.mul_(1/mx)
    test_label_seen = torch.from_numpy(image_label[test_seen_loc]).long()

    # train_label = torch.from_numpy(image_label[train_loc]).long()
...
def generate_syn_feature(generator, classes, attribute, num, device):
    ...
    syn_feature = torch.FloatTensor(nclass*num, 2048)
...
def train():
    ...
    for epoch in range(epochs):
        ...
        train_X = torch.cat((train_feature, syn_feature.to(device)), 0)
        train_Y = torch.cat((train_label, syn_label.to(device)), 0)

        gzsl_cls = classifier.CLASSIFIER(train_X, train_Y, res['test_seen_feature'], res['test_seen_label'], res['test_unseen_feature'], res['test_unseen_label'], seenclasses, unseenclasses, 200, device, 0.001, 0.5, 25, 100, generalized=True,)

        if best_gzsl_acc < gzsl_cls.H:
            best_acc_seen, best_acc_unseen, best_gzsl_acc = gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H
        print("seen: %.4f, unseen: %.4f, H: %.4f" % (gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H))
