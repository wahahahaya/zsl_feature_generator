"""
traditional zsl training, by using the feature from the res101.mat. add the contrastive loss in gan model
"""
...
def train():
    ...
    opt_D = optim.Adam(itertools.chain(net_D.parameters(), net_proj.parameters()), lr=1e-3, betas=(0.5, 0.999))
    ...
    for epoch in range(epochs):
        for iter, (image, label, attribute, feature) in tqdm(list(enumerate(train_loader))):
            ...
            for _ in range(5):
                ...
                embed_real, outz_real = net_proj(input_resv)
                criticD_real = net_D(input_resv, input_attv)
                criticD_real = 10*criticD_real.mean()

                real_ins_contras_loss = loss_con(outz_real, label)
            ...
            embed_real, outz_real = net_proj(input_resv)
            embed_fake, outz_fake = net_proj(fake)

            all_outz = torch.cat((outz_fake, outz_real.detach()), dim=0)
            fake_ins_contras_loss = loss_con(all_outz, torch.cat((label, label), dim=0))

            vae_loss_seen = loss_fn(fake, input_resv, means, log_var)

            criticG_fake = net_D(fake, input_attv).mean()
            errG = vae_loss_seen - 10*criticG_fake + 0.001*fake_ins_contras_loss
            errG.backward()
        ...
        print("Epoch: %d/%d, G loss: %.4f, D loss: %.4f, VEA loss: %.4f, Wasserstein_D: %.4f, real SimCLR loss: %.4f, fake SimCLR loss: %.4f," % (
                    epoch, epochs, G_cost.item(), D_cost.item(), vae_loss_seen.item(), Wasserstein_D, real_ins_contras_loss.item(), fake_ins_contras_loss.item()
            ), end=" "
        )
