...
class Res101(nn.Module):
    def __init__(self):
        ...
        self.conv1x1 = nn.Conv2d(2048, 16, (1, 1), 1)

    def forward(self, x):
        feature_map = self.resnet(x)  # 2048, 8, 8
        feature_map = self.conv1x1(feature_map)  # 16, 8, 8
...
def cal_accuracy(net_cls, net_F, net_linear, data_loader, device):
    ...
    for iteration, (img, label, attribute, feature) in enumerate(data_loader):
        ...
        feature_map = net_F(image)  # (B, 16, 64)
        attribute_map = torch.from_numpy(glove_map).float().to(device)  # (312, 300)
        attribute_map = net_linear(attribute_map)  # (312, 16)

        key = feature_map
        query = attribute_map

        attention_map = torch.einsum('we,bem->bwm', query, key)  # (B, 312, 64)
        B, C1, N2 = attention_map.shape
        feature_attribute = F.max_pool1d(attention_map, 64)
        feature_attribute = feature_attribute.view(B, C1)
...
def train():
    ...
    net_linear = nn.Linear(300, 16).to(device)
